                ...
                {isExpanded && hasExtras && (
                  ...
                  >
                    {extraImages.map((img, i) => {
                      // Wenn dieses Bild vom nächsten Item (mp4) gefolgt wird → als Video-Thumbnail anzeigen
                      const nextIsMp4 = extraImages[i + 1]?.endsWith(".mp4");
                      const prevWasImg = i > 0 && !extraImages[i - 1].endsWith(".mp4") && !extraImages[i - 1].startsWith("youtube:") && extraImages[i].endsWith(".mp4");
                      ...
                      if (isVideoAfterThumb) return null; // wird zusammen mit dem Bild davor gerendert

                      const videoKey = img + "-inline";
                      return (
                        <motion.div
                          key={img}
                        ...
                        >
                          {img.startsWith("youtube:") ? (
                            ...
                            // Bild als Thumbnail — klick startet Video
                            <div className="relative cursor-pointer" onClick={(e) => { e.stopPropagation(); setPlayingInlineVideo(extraImages[i + 1]); }}>
                              {playingInlineVideo === extraImages[i + 1] ? (
                                <video
                                  ...
                                  style={{ borderRadius: RADIUS, backgroundColor: "#000" }}
                                />
                              ) : (
                                <>
                                  <img
